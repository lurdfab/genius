...
def profile(request):
    userproof = Customer.objects.get(user__username = request.user.username)
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, f'dear {new} the following eorror occurred, {form.errors}')
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_1d1d31e79c3be161d0db700de68cfb2eae76938e' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://54.161.192.141/callback' #payment confirmation page
        ...
        #colledt data to send to paystack via call
